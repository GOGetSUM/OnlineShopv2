...
class items_to_orders(db.Model):
...
class OrderModel(db.Model):
    ...
    items = db.relationship("items_to_orders", back_populates="order")

    @property
    def description(self):
        """Simple string description of order, format of "5x Shirt1, 2x Shirt2"""
        items_counts = [f"{i.quantity}x {i.item.name}" for i in self.items]
